...
export function useThreeGame(canvasRef) {
  // Estado para Vue (HUD)
  const score = ref(0)
  const best = ref(getStoredBestScore())
...
let speedBoostTimer = 0
  ...
  if (r < 0.1) {
    ...
    type = 'speed'       // 5% velocidad
  }

  const c = createCoin(type)
  ...
  if (speedBoostTimer <= 0) {
    ...
    const currentSpeed = speedBoostActive.value
  ? speed.value * 1.6
  : speed.value
...
const dz = currentSpeed * dt
    ...
    // ---------- SPAWN OBSTÁCULOS ----------
    obstacleTimer += dt
    const spawnRate = Math.max(0.35, 1.1 - speed.value * 0.015)
